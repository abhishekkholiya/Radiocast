...
export default class MainPostScreen extends React.Component {
 ...
 constructor(props){
    ...
    this.getAudios = async()=>{
        ...
        console.log(this.state.route.params.screename);
        const q =  query(initialQuery, orderBy("title"),where("category", "==",this.props.route.params.screename), limit(this.state.limit));
 ...
 render(){
    return(        
            <View style={Styles.mainview} >    
                    {this.props.route.params.imagename !== 'noimage'?
                         <ImageBackground source={require('../assets/literature.jpeg')} style={{width:windowWidth,height:170,alignItems:'center',justifyContent:'center'}}>
                      ...
                      <View style={{width:windowWidth,height:170,alignItems:'center',justifyContent:'center',backgroundColor:this.props.route.params.color}}>
                      <Text style={{color:'white',fontSize:24}}>{this.props.route.params.screename}</Text>
                      <Text style={{color:'white',fontSize:18,marginTop:10}}>{this.state.listeners} listeners</Text>
                    </View>
                }
