...
let audio_number = 0;
export default class SpecialistPostScreen extends React.Component {
 ...
 constructor(props){
     ...
     this.playAudio = async(title,index)=>{
        ...
        if(this.state.previoustitle !== ''){
                    this.state.audioPlayer.setStatusAsync({shouldPlay:false});
    ...
    this.getAudios = async()=>{
        ...
        const q =  query(initialQuery, orderBy("title"),where("specialist", "==",this.props.route.params.screename), limit(this.state.limit));
            ...
            let documentData2 = querySnapshot.docs.forEach(document =>{ document.data()
                if(document.data()){
                    ...
                    this.setState({
                        ...
                        listeners:this.state.listeners+document.data().listeners
                    });
                }
            });
        }else{
    ...
    this.items = ({item,index})=>{
        return(
            <View  style={Styles.secondview} onPress={async ()=>{
                                try {
                    ...
                    <View style={Styles.secondviewsubview2}>
                        <Text style={Styles.secondviewpodcasterstext}>By {item.podcasters.substring(0,30)}</Text>
    ...
    this.retrieveMore = async ()=>{
        try{
            ...
            const q = query(additionalQuery, limit(this.state.limit), orderBy('title'), where("specialist", "==",this.props.route.params.screename),startAfter(this.state.lastVisible));
 ...
 render(){
    return(        
            <View style={Styles.mainview} >    
                        <ImageBackground source={{uri:this.props.route.params.imagename}} style={{width:windowWidth,height:170,alignItems:'center',justifyContent:'center'}}>
                            <View style={{flexDirection:'row'}}>
                                <Text style={{color:'white',fontSize:24}}>{this.props.route.params.screename}</Text>
                                <Image source={require('../assets/verified.png')} style={{width:26,height:26,marginLeft:2.5}}/>
                                        ...
                                        <TouchableOpacity style={Styles.currentlyplayingview} onPress={()=>{this.setState({showview:true})}}>
                                                    ...
                                                    <View style={{alignSelf:'center',flexDirection:'row',justifyContent:'space-around'}}>
                                                        ...
                                                        <View style={{marginLeft:10,flexDirection:'column',alignSelf:'center'}}>  
                                                            ...
                                                            <Text style={Styles.currentlyplayingtext}>{this.state.currentlyplayingpodcasters.substring(0,30)}</Text>  
                                                        </View>     
                                                    </View>
                                     ...
                                     <View style={Styles.currentlyplayingview2}>   
                                        ...
                                        <View style={{alignSelf:'baseline',flexDirection:'row',width:windowWidth,alignItems:"stretch"}}>  
                                            <View style={{ alignSelf:"baseline",flexDirection:'column',marginTop:15,marginLeft:20}}>
                                                ...
                                                <Text style={{alignSelf:'flex-start',color:'white',fontSize:16}}>By {this.state.currentlyplayingpodcasters.substring(0,30)}</Text>
                                            </View> 
                                        </View>     
                                        <View>    
